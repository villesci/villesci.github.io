...
#require(rnoaa) broken as of 9/9/2026
require(MarineTides)
require(rtide)
require(data.table)
require(tidyverse)
...
#function from the github - appears that rnoaa is broken as of 9/9/2026
...
dover_summ<-dover_2025%>%
  ...
  dplyr::filter(time>hms(start_time) & time<hms(end_time))%>%#filter out times we don't want to work
    dplyr::filter(weekdays(datetime) %in% work_days)%>%#include  days of week we can work
...
# Skip the code: try the app

Don't want to run any R? I turned this workflow into a Shiny app. Pick a tide station, the days and hours you can work, your maximum tide height, and your minimum working period, and it will draw monthly tide calendars and give you a downloadable .ics file (plus PDFs of each month).

::: callout-tip
## [Open the Intertidal Fieldwork Calendar app](https://01a0c633-84db-74c3-25ac-3c08d83ba41c.share.connect.posit.cloud/)
:::

The app uses `rtide` harmonic stations only, so subordinate stations (like Dover Point above) are not available there. The app code is in this post's [`tide-app` folder](https://github.com/villesci/villesci.github.io/tree/python_citation/posts/2025-02-20-tide-calendar/tide-app).

Thanks for reading! Do let me know if you have any comments or suggestions - I'd love to hear them.

create_pub_listing <- function(bib_file, author = "Villeneuve") {
  ...
  articles <- lapply(
    ...
    FUN = function(ibib) {
      ...
      # Use rmarkdown::pandoc_convert instead of system call
      ...
      article <- readLines(output_file)
      article <- article[3:(length(article)-3)]  # Remove header and footer

...
m <- surv.pred*matrix ((10^((ta.mn - rep(ta, each = 100))/z)), nrow = 100)#cut each and nrow to 100 from 1000
out <-0
for(i in 1:length(ta)){
...
colnames(m) <- ta
m <- cbind(surv.prob=seq(1,0.01,-0.01),m) #cut survival prob steps to 0.01
...
plot_3dC<-plot_ly(z=tryme,x=label_temp,y=label_time,colors="RdYlBu")%>%add_surface(contours= list(
  z = list(
    show=TRUE,
    usecolormap=TRUE,
    ...
    size = 0.1)))%>%
  layout(scene = list(xaxis=list(title="Temperature"),yaxis=list(type="log",title="Time (min)"),zaxis=list(title="Survival")))

```

```{r,echo=F,warning=F}

frameWidget(plot_3dC,width="100%")


```

*Interactive Plotly Widget takes its time to load!*
